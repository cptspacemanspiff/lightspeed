...
impl TranscriptState {
    ...
    fn push_committed_cell(&mut self, cell: Box<dyn ChatCell>) {
        self.push_tool_block_spacer_before(cell.as_ref());
    ...
    fn push_tool_block_spacer_before(&mut self, next: &dyn ChatCell) {
        if next.kind() == ChatCellKind::ToolChain
            || !self
                .cells
                .last()
                .is_some_and(|cell| cell.kind() == ChatCellKind::ToolChain)
        ...
        let spacer = Box::new(NoticeCell::blank(format!(
            "tool-block-spacer-before:{}",
            next.id()
        )));
        let id = spacer.id().to_string();
        let fingerprint = cell_fingerprint(spacer.as_ref());
        let already_committed = self.is_emitted_history_cell(&(id, fingerprint));
        self.cells.push(spacer);
        if !already_committed {
            self.pending_history_cell_indices
                .push(self.cells.len().saturating_sub(1));
        }
...
mod tests {
    ...
    #[test]
    fn replace_turns_emits_only_new_tool_batch_cells() {
        fn chain(id: &str, title: &str) -> ChatToolChainView {
            ChatToolChainView {
                id: id.into(),
                title: title.into(),
                status: ChatProgressStatus::Succeeded,
        ...
        let mut state = TranscriptState::default();
        let first = chain("run-1:1", "tools 1 calls");
        state.apply_chat_event(ChatEvent::TranscriptDelta(ChatDelta::ReplaceTurns {
            session_id: "s-1".into(),
            turns: vec![ChatTurn {
                turn_id: "turn-1".into(),
        ...
        assert!(first_lines[0].contains("tools 1 calls"));
        assert!(first_lines[1].contains("glob"));
        ...
        state.apply_chat_event(ChatEvent::TranscriptDelta(ChatDelta::ReplaceTurns {
            ...
            turns: vec![ChatTurn {
                ...
                assistant: None,
                run: None,
                tool_chains: vec![first, chain("run-1:2", "tools 5 calls")],
            }],
        }));
        let next_lines = state
            ...
            .map(|line| line.to_string())
            .collect::<Vec<_>>();
        assert_eq!(next_lines.len(), 2);
        assert!(next_lines[0].contains("tools 5 calls"));
        assert!(next_lines[1].contains("glob"));
    }

    #[test]
    fn consecutive_terminal_tool_batches_share_one_block_until_next_non_tool_cell() {
        ...
        assert!(first_batch[1].contains("glob"));
        ...
        assert!(second_batch[1].contains("glob"));
